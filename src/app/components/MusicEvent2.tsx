...
export default function MusicSection() {
  ...
  return (
    ...
    >
      {/* Foreground Text - positioned differently for mobile vs desktop */}
      <div className="relative z-20 w-full text-center mt-0 md:mt-12 lg:mt-12 flex flex-col justify-center items-center md:block md:justify-start md:items-start h-full md:h-auto">
        <h2
          ref={headingRef}
          className="text-white text-3xl md:text-6xl font-serif mb-3 font-['BL-Melody'] mt-0 md:mt-16 lg:mt-16 max-w-xs md:max-w-none leading-tight"
        >
          Where the music never stops and<br className="hidden md:block"/>the glasses never empty.
        </h2>
        <p
          ref={paragraphRef}
          className="text-white mt-2 md:mt-5 text-xl md:text-2xl font-serif font-lora"
        >
      ...
      {floatingImages.map((imgData, index) => (
        <div
          ...
          style={{
            width: isMobile ? `${Math.round(imgData.width * 0.6)}px` : `${imgData.width}px`,
            height: isMobile ? `${Math.round(imgData.height * 0.6)}px` : `${imgData.height}px`,
            top: imgData.top,
            left: imgData.left,
            zIndex: 0,
      ...
      {/* Vinyl Center - smaller on mobile */}
      <div
        ref={vinylRef}
        className="absolute w-[400px] h-[400px] md:w-[1000px] md:h-[1000px] bg-center bg-no-repeat bg-contain z-10"
        style={{
          top: "50%",
          left: "50%",
